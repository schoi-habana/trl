...
def is_from_value_conversational(example: dict[str, Any]) -> bool:
    """
    Check if the example is in a conversational format with keys "from" and "value".

    We recommend not using this format, and use ChatML instead.
    ...
    ```python
    >>> example = {"prompt": [{"from": "user", "value": "What color is the sky?"}]}
    >>> is_from_value_conversational(example)
    True
    >>> example = {"prompt": "The sky is"})
    >>> is_from_value_conversational(example)
    False
    ```
    """
    supported_keys = ["prompt", "chosen", "rejected", "completion", "conversations"]
    example_keys = {key for key in example.keys() if key in supported_keys}
